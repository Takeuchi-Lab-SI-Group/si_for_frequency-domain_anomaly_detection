...
class SimulatedAnnealing:
    ...
    def perform_sa(
        self, tempinit_init=1000, temp_factor=1.5, loop_param=1, accepatance_rate_init=0.5, cooling_rate=0.8
    ):
        ...
        while True:
            ...
            self.update_univariate(
                list(self.cp_list), list(self.cost_list), list(self.K_d_list), 
                self.cp_set, rng, iteration, calc_init_temp=True
            ) 
        ...
        while True: 
            self.accept = self.reject = 0
            self.update_univariate(
                list(self.cp_list), list(self.cost_list), list(self.K_d_list), self.cp_set, rng, iteration
            )
            self.update_multivariate(
                list(self.cp_list), list(self.cost_list), list(self.K_d_list), self.cp_set, rng
            )
            if self.accept == 0:
                break
    ...
    def update_univariate(
        ...
        while step < iteration: 
            ...
            cost_cand = cost_func.calc_cost(
                self.F[d_index_cand], self.freq[d_index_cand], cp_d_list_cand, self.window
            )
            K_d_cand = len(cp_d_list_cand) - 2
            cp_list_cand = list(cp_list)
            cp_list_cand[d_index_cand] = cp_d_list_cand
            cp_set_cand = len(set(sum([cp_list_cand[i] for i in self.d_index_dp_list], []))) - 2
            ...
            cp_d_list_update, delta_status = self.update_solution(cp_d_list_cand, delta, theta)

            if self.SI_mode:
                self.selectionevent_sa(
                    [d_index_cand], cp_list, cp_list_cand, penalty, theta, 
                    delta_status, self.a, self.b, self.FFT_mat_a, self.FFT_mat_b
                )
    ...
    def selectionevent_sa(
        self, d_index_list, cp_list, cp_list_cand, penalty, theta, delta_status, a, b, FFT_mat_a, FFT_mat_b
    ):
        ...
        if len(set(d_list) & set(self.freq_tau_obs)) != 0:
            ...
            for i, d in zip(d_index_list, d_list): 
                ...
                cost_mat_sa.append(utils.mat_diff(cost_mat_series_cand, cost_mat_series))

            metropolis_threshold = self.temp * np.log(theta) 

            if delta_status == "accept":
                self.interval_ts = polytope_to_interval(
                    self.interval_ts, d_list, cost_mat_sa, self.freq_tau_obs, 
                    penalty, a, b, FFT_mat_a, FFT_mat_b, metropolis_threshold
                )
            if delta_status == "reject":
                self.interval_ts = polytope_to_interval(
                    self.interval_ts, d_list, [mat * -1 for mat in cost_mat_sa], self.freq_tau_obs, 
                    -penalty, a, b, FFT_mat_a, FFT_mat_b, -metropolis_threshold
                )
    ...
    def update_multivariate(self, cp_list, cost_list, K_d_list, cp_set, rng):
        ...
        for i in d_index_list:
            cost_cand = cost_func.calc_cost(self.F[i], self.freq[i], cp_list_cand[i], self.window)
        ...
        if self.SI_mode:
            self.selectionevent_sa(
                d_index_list, cp_list, cp_list_cand, penalty, theta, 
                delta_status, self.a, self.b, self.FFT_mat_a, self.FFT_mat_b
            )
    ...
    def merge_changepoint(self, cp_list, rng):
        ...
        cp_set_list = sorted(list(set(sum([cp_list_cand[i] for i in self.d_index_dp_list], [])))) 
