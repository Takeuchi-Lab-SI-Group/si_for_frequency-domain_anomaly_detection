...
class HypothesisTesting:
    ...
    def projection_mat_coef_d(self, d, left, right):
        ...
        P_coef_d[self.tau - left : self.tau, self.tau - left : self.tau] = chi_coef / (left ** 2)
        P_coef_d[self.tau - left : self.tau, self.tau : self.tau + right] = -chi_coef / (left * right)
        P_coef_d[self.tau : self.tau + right, self.tau-left : self.tau] = -chi_coef / (left * right)
        P_coef_d[self.tau : self.tau + right, self.tau : self.tau + right] = chi_coef / (right ** 2)
    ...
    def calc_FFT_mat_ab(self):
        ...
        for d in range(freq_all):
            ...
            if d == 0 or d == self.window / 2:
                ...
                FFT_mat_a_d = utils.Roth_column_lemma_sum(a_vec_inverse, FFT_d_r, FFT_d_i) 
                FFT_mat_b_d = utils.Roth_column_lemma_sum(b_vec_inverse, FFT_d_r, FFT_d_i)
...
def poly_lt_zero(alpha, beta, gamma, tol=1e-10):
    ...
    if alpha == 0:
        if beta == 0:
            if gamma > 0:
                raise ValueError("Test direction of interest does not intersect with the inequality.")
        elif beta < 0:
