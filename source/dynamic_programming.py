...
class DynamicProgramming:
    ...
    def perform_dp(self):
        ...
        for i, (F_d, d) in enumerate(zip(self.F, self.freq)): 
            ...
            self.loss_min_list = [-self.beta_d, 0.]
            self.cp_t_list = [[], [1]] 
        ...
        return cp_list, cost_list, K_d_list, cp_set, loss_dp, d_index_dp_list, self.interval_ts
    ...
    def selectionevent_dp(self, selected_index, t, d):
        ...
        for i in range(t):
            ...
            self.interval_ts = polytope_to_interval(
                self.interval_ts, [d], [cost_mat], self.freq_tau_obs, 
                penalty, self.a, self.b, self.FFT_mat_a, self.FFT_mat_b
            )
