...
class SelectiveInferenceChi(SelectiveInference):
    def __init__(self, stat, degree, a, b):
        ...
        self.limits = RealSubset([[self.mode - 20.0, np.max([self.stat, self.mode]) + 10.0]]) & self.support
...
class ChangepointInference(ChangepointDetection):
    def inference(self, tau, sigma_inference, parametric=True):
        ...
        self.tau = tau
        self.freq_tau = [d for i, d in enumerate(self.F_freq[1]) if self.tau in self.cp_list_sa[i]]
        ...
        testing = HypothesisTesting(
            self.tau, self.freq_tau, self.F_freq, sigma_inference, self.window, self.FFT_mat, self.cp_list_sa
        )
    ...
    def algorithm(self, a, b, z):
        ...
        CP_detection = ChangepointDetection(
            x, self.sigma, self.window, self.freq_l, self.freq_h, self.seed, a, b, 
            self.FFT_mat_a, self.FFT_mat_b, self.freq_tau, SI_mode=True
        )
