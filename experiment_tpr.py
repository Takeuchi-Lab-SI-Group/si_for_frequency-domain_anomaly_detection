...
def get_p_value(parameters):
    ...
    for f in range(number_of_freq):
        ...
        mu[boundary_1:boundary_2] += (amplitude[f] + delta) * np.sin(omega[f] * t[boundary_1:boundary_2])
        mu[boundary_2:] += (amplitude[f] + 2 * delta) * np.sin(omega[f] * t[boundary_2:])
    ...
    tau_1 = int(T / 3)
    tau_2 = int(2 * T / 3)
    tau_cand_all = [[tau_1-2, tau_1-1, tau_1, tau_1+1, tau_1+2], [tau_2-2, tau_2-1, tau_2, tau_2+1, tau_2+2]]
    ...
    for index in tau_cand_index:
        ...
        for tau in tau_cand_perm:
            freq_tau = [d for i, d in enumerate(CP_detection.F_freq[1]) if tau in cp_list_sa[i]] 

            if set(freq_tau).issubset(set(freq_index)) and len(freq_tau) >= 1:
                tau_range = [tau_cand[num*step] for d in freq_tau for num, f in enumerate(freq_index) if f == d]
